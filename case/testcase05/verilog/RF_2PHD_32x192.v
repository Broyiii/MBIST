...
//                                                                    
//   Memory Name:RF_2PHD_32x192                                       
//   Memory Size:32 words x 192 bits                                  
//                                                                    
//                               PORT NAME                            
//                               ---------                            
//               Output Ports                                         
...
module RF_2PHD_32x192 ( RSCOUT, QB, ADRA, DA, WEA, MEA, RSCIN, RSCEN, RSCRST, RSCLK, FISO, CLKA, TEST1A, RMEA, RMA, LS, ADRB, MEB, CLKB, TEST1B, RMEB, RMB);
...
wire ADRA_valid;
assign ADRA_valid = ( ( ADRA_pre < 32 )?1'b1:1'b0);


wire ADRB_valid;
assign ADRB_valid = ( ( ADRB_pre < 32 )?1'b1:1'b0);
...
begin
  ...
  begin
    ...
    begin
      $display("<<VIRL_MEM_WARNING: RMA[3] is unknown>> at time=%t; instance=%m (RF_2PHD_32x192_core)",$realtime);
    end
  end
end
always @(RMB_buf[3])
begin
  ...
  begin
    if( (MES_CNTRL=="ON"|| MES_CNTRL=="WARN") && $realtime != 0 && uut.mes_all_valid)
    begin
      $display("<<VIRL_MEM_WARNING: RMB[3] is unknown>> at time=%t; instance=%m (RF_2PHD_32x192_core)",$realtime);
    end
  end
end
...
parameter words = 32, bits = 192, addrbits = 5, timingmode = 1, rmbits=3;
...
wire RSCOUT_tmp;
reg mes_all_valid;
reg [199:0] mem_core_array [0:31];
reg [191:0] mem_core_array_read [0:31];
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((start_adr_cnt + num_adr_cnt -1) > 31) begin
        $display("<<VIRL_MEM_WARNING>> \"init_mem_start_adr=%d + num_of_locations=%d\" exceed maximum address (NW-1)=31 instance=%m ",start_adr_cnt,num_adr_cnt);
      end
      if (^init_value === 1'bX)
      begin
      ...
      begin
        for( adr_cnt = 0; adr_cnt < 32 ; adr_cnt = adr_cnt + 1)
        begin
      ...
      begin
        ...
        begin
          if(( MES_CNTRL=="ON" || MES_CNTRL=="WARN")&& $realtime != 0 && mes_all_valid == 1'b1)
            $display("<<VIRL_MEM_WARNING:address ADRA is out of range>>, RANGE:0 to 31 time=%0t instance=%m\n",$time);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if(( MES_CNTRL=="ON" || MES_CNTRL=="WARN")&& $realtime != 0 && mes_all_valid == 1'b1)
            $display("<<VIRL_MEM_WARNING:address ADRB is out of range>>, RANGE:0 to 31 time=%0t instance=%m\n",$time);
