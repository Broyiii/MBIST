...
//                                                                    
//   Memory Name:RF_2PHD_512x21                                       
//   Memory Size:512 words x 21 bits                                  
//                                                                    
//                               PORT NAME                            
//                               ---------                            
//               Output Ports                                         
...
module RF_2PHD_512x21 ( RSCOUT, QB, ADRA, DA, WEA, MEA, RSCIN, RSCEN, RSCRST, RSCLK, FISO, CLKA, TEST1A, RMEA, RMA, LS, ADRB, MEB, CLKB, TEST1B, RMEB, RMB);
...
reg notif_adra;
wire [20:0] notif_da;
reg notif_da_20, notif_da_19, notif_da_18, notif_da_17, notif_da_16, notif_da_15, notif_da_14, notif_da_13, notif_da_12, notif_da_11, notif_da_10, notif_da_9, notif_da_8, notif_da_7, notif_da_6, notif_da_5, notif_da_4, notif_da_3, notif_da_2, notif_da_1, notif_da_0;
reg notif_wea;
reg notif_mea;
reg notif_rscin;
reg notif_rscen;
...
`ifdef VIRAGE_FAST_VERILOG
`else
assign notif_da = {notif_da_20, notif_da_19, notif_da_18, notif_da_17, notif_da_16, notif_da_15, notif_da_14, notif_da_13, notif_da_12, notif_da_11, notif_da_10, notif_da_9, notif_da_8, notif_da_7, notif_da_6, notif_da_5, notif_da_4, notif_da_3, notif_da_2, notif_da_1, notif_da_0};
...
begin
  ...
  begin
    ...
    begin
      $display("<<VIRL_MEM_WARNING: RMA[3] is unknown>> at time=%t; instance=%m (RF_2PHD_512x21_core)",$realtime);
    end
  end
end
always @(RMB_buf[3])
begin
  ...
  begin
    if( (MES_CNTRL=="ON"|| MES_CNTRL=="WARN") && $realtime != 0 && uut.mes_all_valid)
    begin
      $display("<<VIRL_MEM_WARNING: RMB[3] is unknown>> at time=%t; instance=%m (RF_2PHD_512x21_core)",$realtime);
    end
  end
end
...
parameter words = 512, bits = 21, addrbits = 9, timingmode = 1, rmbits=3;
...
wire RSCOUT_tmp;
reg mes_all_valid;
reg [91:0] mem_core_array [0:127];
reg [20:0] mem_core_array_read [0:511];
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          blk_right = ((bit_index - 11) * 1) + (Y_adr/4);
  ...
  begin
    ...
    begin
      ...
      begin
      if ({scan_reg[4],scan_reg[5],scan_reg[6],scan_reg[7]} > 4'b1010)
      begin
        if ((MES_CNTRL=="ON" || MES_CNTRL=="ERR") && $realtime != 0 && mes_all_valid)
        begin
        ...
        end // if of {scan_reg[4],scan_reg[5],scan_reg[6],scan_reg[7]} > 4'b1010
        else
        begin
        ...
        end // else of if {scan_reg[4],scan_reg[5],scan_reg[6],scan_reg[7]} > 4'b1010
      end // if of unused scan register bits are zero
      else
      begin
        if ((MES_CNTRL=="ON" || MES_CNTRL=="ERR") && $realtime != 0 && mes_all_valid)
...
begin
  ...
  begin
    ...
    begin
//Left
      memrow[47:0] = tmp_memrow[47:0];
      if (!left_col_out_range)
      begin
        for (bit_cnt = 0; bit_cnt < 11; bit_cnt = bit_cnt +1)
        begin
...
//Right
      memrow[91:48] = tmp_memrow[91:48];
      if (!right_col_out_range)
      begin
        for (bit_cnt = 11; bit_cnt < 21; bit_cnt = bit_cnt +1)
        begin
          get_column_position (col_address, Y_adr, bit_cnt);
          memrow[col_address] = data[bit_cnt]; 
        end
...
begin
  ...
  begin
    ...
    begin
      for ( j = 0; j < 4 ; j = j +1 )
      begin
        column_to_corrupt = ((4*(faulty_blk_right+13)) + j);
...
begin
  ...
  Y_adr = address[1:0];
   ...
   begin
     memdata[10:0] = {11{1'bx}};
   end
   get_column_position (col_address, Y_adr, 11);
   if ( right_col_out_range == 1'b0 )
   begin
     for (bit_cnt = 11; bit_cnt < 21; bit_cnt = bit_cnt +1)
     begin
 ...
 begin
  ...
  if( flag_range_ok == `True)
   begin
    ...
    begin
      mem_core_array[row_index] = 92'bx;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (init_value === 21'b0 || init_value === 21'b1)
      begin
        for( adr_cnt = 0; adr_cnt < 512 ; adr_cnt = adr_cnt + 1)
        begin
          mem_array[adr_cnt] = {21{init_value[0]}};
        end
      end
      else
      begin
        ...
        begin
          ...
          begin
            mem_array[adr_cnt] = i;
            i = i + 1;
            if (i > 21'b111111111111111111111) i = 0;
          end
        end
      end
    end
...
genvar nk;
generate for (nk=0; nk<21; nk=nk+1)
begin : ENDA_blk
